...
>  
> With Dave’s departure, we are thrilled to announce that Mark is stepping into the role of Chief Information Security Officer (CISO). Mark has demonstrated immense passion for our overarching vision, and this transition frees him from day-to-day client accounts to focus exclusively on macro-level security architecture, enterprise risk governance, and public-readiness compliance. Given the critical nature of this strategic mandate, we have concluded his previous operational development framework early so he can fully focus on building institutional trust at the C-suite level. Please join us in congratulating Mark on this vital milestone.
...
- **"Frees him from client accounts to focus on macro-level architecture (Mark)"** → **Kicked Upstairs / Containment**: Removed from revenue roles due to client complaints; placed in a powerless CISO title with zero direct reports.
